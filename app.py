...
if "items_alquiler" not in st.session_state:
...
def dialog_registrar_alquiler():
    ...
    with col_f1:
        fecha_alq = st.date_input("Fecha de Alquiler", value=date.today(), key="alq_fecha")
    with col_f2:
    ...
    if st.button("💾 GUARDAR ALQUILER", type="primary", use_container_width=True):
        if not cli_nombre.strip():
            ...
            supabase.table("eventos").insert({
                ...
                "tipo": "Alquiler Mobiliario",
                "cliente": cli_nombre.strip(),
                "telefono": cli_telefono.strip(),
...
def dialog_ver_ficha(evento):
    ...
    header_class, color_fondo, nombre_marca = ("header-mobiliario", "#FFD6D6", "MOBILIARIO") if "mobiliario" in marca else (("header-alquiler", "#A2D2FF", "LOCAL") if "local" in marca else (("header-risuena", "#B7E4C7", "RISUEÑA") if "risueña" in marca else ("header-madai", "#E0B0FF", "MADAI")))
    ...
    for idx, ev in enumerate(lista_eventos):
        with cols[idx % 2]:
            ...
            if "mobiliario" in marca_raw:
                ...
                card_class, card_bg, badge_class, nombre_marca_tag = "card-alquiler", "#A2D2FF", "badge-local", "local"
            elif "risueña" in marca_raw:
                card_class, card_bg, badge_class, nombre_marca_tag = "card-risuena", "#B7E4C7", "badge-risuena", "risueña"
            else:
                card_class, card_bg, badge_class, nombre_marca_tag = "card-madai", "#E0B0FF", "badge-madai", "madai"
            ...
            if "mobiliario" in marca_raw:
                html_tarjeta += f'<div class="data-line" style="color:#D90429; font-weight:bold;">📦 MODO LECTURA - ALQUILER MOBILIARIO</div>'
            else:
                html_tarjeta += (
